...
} from './user-notifications-hub.service';
...
export class UserNotificationsHubController {
  constructor(
    private readonly notificationHubService: UserNotificationsHubService,
  ) {}

  @Get()
  @ApiOperation({
    summary: 'Pull system logs data array containing user history messages',
  })
  ...
  @ApiOperation({
    summary:
      'Isolate structural properties belonging to a specific item log identity',
  })
  async getSingleNotification(
    @Req() req: any,
    @Param('notifId') notifId: string,
  ) {
    return this.notificationHubService.getSingleNotification(
      req.user.id,
      notifId,
    );
  }
  ...
  @Patch('mark-all-read')
  @ApiOperation({
    summary: 'Initialize clear parameter loop across all assigned user rows',
  })
  ...
  @ApiOperation({
    summary:
      'Update state variables tracking read metadata parameters for a record row',
  })
