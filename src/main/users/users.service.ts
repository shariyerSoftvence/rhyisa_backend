...
export class UserProfileService {
  ...
  async createProfile(
    ...
    try {
      ...
      const uploadedImage = await this.uploadFilesService.uploadSingleImage(
        file,
        'users/avatars',
      );
      ...
      try {
        aiGoals = await this.openaiService.generateDailyHealthGoals({
          age: dto.age,
          gender: dto.gender,
          height: dto.height,
          weight: dto.weight,
  ...
  async getProfileByAuthId(authId: string) {
    try {
      ...
      throw new InternalServerErrorException(
        'Failed to pull target health profile metrics structural record',
      );
    }
  }
  ...
  async updateProfile(
    ...
    try {
      ...
      if (dto.currentActivityLevel !== undefined) {
        updateData.currentActivityLevel = dto.currentActivityLevel;
      }

      if (dto.currentDiet !== undefined) {
        updateData.currentDiet = dto.currentDiet;
      }

      if (dto.primaryGoal !== undefined) {
        updateData.primaryGoal = dto.primaryGoal;
      }

      if (dto.motivationLevel !== undefined) {
        updateData.motivationLevel = dto.motivationLevel;
      }

      if (dto.hasHealthCondition !== undefined) {
        updateData.hasHealthCondition = dto.hasHealthCondition;
      }

      if (dto.supplements !== undefined) {
        updateData.supplements = dto.supplements;
      }
        ...
        if (shouldRegenerateGoals) {
          ...
          try {
            aiGoals = await this.openaiService.generateDailyHealthGoals({
              ...
              currentActivityLevel: updatedProfile.currentActivityLevel!,
              currentDiet: updatedProfile.currentDiet!,
              primaryGoal: updatedProfile.primaryGoal,
              motivationLevel: updatedProfile.motivationLevel!,
              supplements: updatedProfile.supplements,
            });
          } catch (error) {
            aiGoals = {
              calorieGoal: 2200,
          ...
          updatedGoals = await tx.userHealthGoal.update({
            where: {
  ...
  async selfDeleteAccount(authId: string) {
    try {
      ...
      if (!userAccount) {
        throw new NotFoundException(
          'Target account record missing structural database key alignment indices',
        );
      }
      ...
      return {
        message:
          'Your account status profile has been successfully moved to DELETED state configuration metrics',
      };
    } catch (error) {
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException(
        'Failed to process account closure operational routine tasks',
      );
    }
  }
}
